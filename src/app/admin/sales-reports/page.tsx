...
import { TrendingUpIcon, PackageIcon, ClipboardIcon } from '@/components/Icons';
...
export default function ReportsPage() {
    const { token } = useAuth();
    const [activeTab, setActiveTab] = useState<'sales' | 'visiting'>('sales');
    ...
    const renderSalesReport = () => {
        ...
        if (!data) return <div className={styles.loading}><p>No data available</p></div>;
    ...
    const renderVisitingReport = () => (
        <div className={styles.content}>
            <div className={styles.emptyState}>
                <span className={styles.emptyIcon}>📋</span>
                <h3>No visiting reports yet</h3>
                <p>Create a report to track site visits and customer interactions.</p>
                <button className="btn btn-primary" style={{ marginTop: '1.5rem' }} onClick={() => alert('Add Report feature coming soon!')}>
                    + Add Report
                </button>
            </div>
        </div>
        ...
        <div className={styles.page}>
            <div className={styles.header}>
                <div>
                    ...
                    <p className={styles.subtitle}>Sales analytics and visiting reports</p>
                </div>
            </div>
            ...
            <div className={styles.tabs}>
                ...
                <button
                    className={`${styles.tab} ${activeTab === 'visiting' ? styles.tabActive : ''}`}
            ...
            <div className={styles.tabContent}>
                {activeTab === 'sales' ? renderSalesReport() : renderVisitingReport()}
            </div>
        </div>
    );
}
